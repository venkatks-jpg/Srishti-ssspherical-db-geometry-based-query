...
Schema-aligned to aria_setup.py v5.0:
  ...
  - CLUSTER_STEP theta assignment matches setup script

Changes from v1.0:
  - Fixed subdomain JOIN: domain_name TEXT (was domain_id INTEGER)
  - Fixed theta assignment: parent_r + (count+1)*CLUSTER_STEP (matches setup)
...
SILENT_EXT = {
    ...
    ".mdb", ".accdb", ".mdf", ".ldf", ".dbf", ".ora", ".dmp",
    ".dll", ".exe", ".so", ".o", ".a",
    ".sys", ".cat", ".inf", ".drv",
    ".msi", ".reg", ".bat", ".cmd", ".ps1",
    ".gguf", ".bin", ".pt", ".pth", ".safetensors", ".ckpt",
    ".pkl", ".pickle",
    ".xml", ".xsd", ".css", ".less", ".scss",
    ".pem", ".key", ".crt", ".csr",
    ".cfg", ".ini", ".conf", ".log",
}
...
# ── IMAGE EXTENSIONS — direct to pictures_paintings ──────────────────────────
IMAGE_EXT = {
...
# ── COMPACT EXTENSIONS — direct to compacted_files ───────────────────────────
COMPACT_EXT = {
...
def classify_subdomain(domain, path, name, content=""):
    """Assign subdomain within a known domain. Whole-word matching."""
    subdomain_hints = {
        "physics": {
...
# ── THETA ASSIGNMENT — aligned to aria_setup.py v5.0 ─────────────────────────
def get_or_assign_theta(conn, domain, subdomain):
    """
    Assign theta using parent_r + (count+1) * CLUSTER_STEP.
    Subdomains table uses domain_name TEXT — matches aria_setup.py v5.0.
    Caches in memory to avoid repeated DB reads.
    """
    global _theta_cache, _theta_counter

    if domain not in _theta_cache:
        # Load existing subdomains for this domain from DB
        rows = conn.execute("""
            SELECT name, theta FROM subdomains
            WHERE domain_name = ?
        """, (domain,)).fetchall()
        _theta_cache[domain]   = {r[0]: r[1] for r in rows}
        _theta_counter[domain] = len(rows)  # count of existing subdomains

    if subdomain in _theta_cache[domain]:
        return _theta_cache[domain][subdomain]

    # New subdomain — compute theta as setup.py does
    # theta = parent_r + (count + 1) * CLUSTER_STEP
    parent_r = DOMAIN_R.get(domain, PHI)
    new_count = _theta_counter[domain] + 1
    new_theta = parent_r + (new_count * CLUSTER_STEP)
    ...
    _theta_counter[domain]           = new_count
    return new_theta
...
S bogom.
    ...
    print(f"  phi = {PHI:.16f}")
    print(f"  Schema  : aligned to aria_setup.py v5.0")
    print(f"  Matcher : whole-word (no substring collisions)")
    print(f"  Theta   : parent_r + (n * CLUSTER_STEP)")
    print("="*58 + "\n")
