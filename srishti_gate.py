#!/usr/bin/env python3
"""
srishti_gate.py  —  Srishti Spherical Query Server  v1.0
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
DB1_PATH = str(Path.home() / "srishti1.db")   # DATA 1
DB2_PATH = str(Path.home() / "srishti3.db")   # DATA3 — production
PORT     = 7509
HOST     = "0.0.0.0"
...
def query_db(params, db_path=None):
    ...
    Step 2 — Fix theta (optional).
              If subdomain given, look up its theta from subdomains table.
              Add AND theta BETWEEN (theta - CLUSTER_STEP) AND
              (theta + CLUSTER_STEP).
    ...
    if lang == "tamil" or (text and any(
        ...
        # ── Step 2 — Fix theta (subdomain band) ────────────────────────────
        if subdomain and domain:
            row = conn.execute(
                "SELECT theta FROM subdomains "
                "WHERE domain_name = ? AND name = ?",
                (domain, subdomain)
            ).fetchone()
            if row:
                band = 0.0001
                clauses.append("theta BETWEEN ? AND ?")
                qparams += [row[0] - band, row[0] + band]
...
class SrishtiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/health":
            ...
            self.end_headers()
            try:
                c1 = sqlite3.connect(DB1_PATH).execute(
                    "SELECT COUNT(*) FROM files"
                ).fetchone()[0]
                c2 = sqlite3.connect(DB2_PATH).execute(
                    "SELECT COUNT(*) FROM files"
                ).fetchone()[0]
            except Exception:
                c1 = c2 = 0
...
def main():
    ...
    for db, label in [(DB1_PATH, "DB1"), (DB2_PATH, "DB2")]:
        if not Path(db).exists():
            print(f"  Warning: {label} not found at {db}")
    ...
    print(f"  Files   : {counts.get('DB1', 0):,} (DB1) "
          f"| {counts.get('DB2', 0):,} (DB2)")
    print(f"  Port    : {PORT}")
    print(f"  \u03c6       : {PHI}")
