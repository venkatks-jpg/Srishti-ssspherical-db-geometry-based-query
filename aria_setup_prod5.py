#!/usr/bin/env python3
"""
aria_setup_prod5.py  —  Srishti Spherical DB Indexer  v5.0
...
SPHERICAL GEOMETRY:
    ...
    theta =  parent_r + ( subdomain_index * CLUSTER_STEP )
              Subdomains cluster just above parent r, like puppies around mother
...
USAGE:
    ...
    python3 aria_setup_prod5.py --add-domain law criminal,civil,family --db ~/srishti3.db
    python3 aria_setup_prod5.py --add-subdomain physics plasma_physics --keywords plasma,tokamak --db ~/srishti3.db
"""
...
# ── DOMAIN NAME SET — loaded at startup from DOMAINS list ─────────────────────
_DOMAIN_NAMES = {d[1] for d in DOMAINS}
...
def setup_db(db_path):
    ...
    for parent_domain, sub_name, keywords in SUBDOMAINS:
        _assign_subdomain(conn, parent_domain, sub_name, keywords, now)
...
def _assign_subdomain(conn, domain_name, subdomain_name, keywords, ts=None):
    ...
    theta = parent_r + ((count + 1) * CLUSTER_STEP)
    try:
        conn.execute(
...
def classify_domain(conn, filepath, filename, content):
    ...
    if not scores:
        # Level 4 — honest unclassified. No fallback. No guessing.
        return "unclassified", 0.0, 0.1

    best       = max(scores, key=lambda k: scores[k][0])
    total      = sum(v[0] for v in scores.values()) or 1
    confidence = round(scores[best][0] / total, 3)

    # Confidence threshold — if best score is too weak, go unclassified
    # A single weak hit should not force classification
    if scores[best][0] < 2 and confidence < 0.4:
        return "unclassified", 0.0, confidence

    return best, scores[best][1], confidence


def classify_subdomain(conn, domain_name, filename, content):
...
The universe is alongside.
    ...
    print(f"  phi = {PHI:.16f}")
    print(f"  Classification : path-first, whole-word, honest unclassified")
